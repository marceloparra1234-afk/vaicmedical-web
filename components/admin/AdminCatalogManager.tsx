    ...
    <div className="fixed inset-0 z-50 grid place-items-center bg-[#213255]/55 p-5">
      <section className="max-h-[90vh] w-full max-w-2xl overflow-auto bg-white p-6 shadow-2xl">
        ...
        <div className="mt-6 grid gap-4">
          ...
          <Field label="Sublíneas separadas por coma" onChange={(value) => setSublines(value.split(",").map((item) => item.trim()).filter(Boolean))} value={sublines.join(", ")} />
          <Field label="Orden visual" onChange={(value) => setOrder(Number(value))} type="number" value={String(order)} />
          <label className="text-xs font-bold text-[#34466f]">Imagen principal<input accept="image/*" className="mt-2 block w-full text-xs" onChange={async (event) => { const file = event.target.files?.[0]; if (file) setImage(await uploadImage(file)); }} type="file" /></label>
          {image && <div><div className="h-36 bg-[#eaf8fc] bg-contain bg-center bg-no-repeat" style={{ backgroundImage: `url("${image}")` }} /><button className="mt-2 text-xs font-bold text-[#213255]" onClick={() => setImage("")} type="button">Quitar imagen</button></div>}
